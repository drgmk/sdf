...
class ConvolvedModel(object):
    ...
    def read_file(cls, file):
        ...
        if type_ != 'convolvedmodel':
            raise utils.SdfError("file {} not a convolvedmodel, is a {}".format(file, type_))
        ...
        if self.param_values is not None and value is not None:
            for i, key in enumerate(self.parameters):
                if len(self.param_values[key]) != value.shape[i]:
                    raise utils.SdfError("expected dimension {} to have"
                                         " size {} but got {}".format(i, len(self.param_values[key]), value.shape[i]))
